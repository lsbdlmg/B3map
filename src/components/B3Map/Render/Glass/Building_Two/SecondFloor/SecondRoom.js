...
const create = (Objects, device, world, RAPIER) => {
  ...
  const Z_OFFSET = -197
  {
    // 靠走廊窗户
    {
      ...
      Glass.positionArray = []
      for (let i = 0; i < 6; i++) Glass.positionArray.push({ x: -650.5, y: 12 + Y_OFFSET, z: -40 - i * 10 + Z_OFFSET })
      for (let i = 0; i < 6; i++) Glass.positionArray.push({ x: -650.5, y: 12 + Y_OFFSET, z: -107 - i * 10 + Z_OFFSET })
