...
const create = (Objects, device, world, RAPIER) => {
  {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const glassZ = 44 + i * 9
      // 玻璃 下 (1-7)
      Glass.positionArray.push({ x: -886, y: 38, z: glassZ })
      Glass.scaleArray.push({ x: 0.1, y: 3, z: 4 })
      // 玻璃 中 (8-18)
      Glass.positionArray.push({ x: -886, y: 47, z: glassZ })
      Glass.scaleArray.push({ x: 0.1, y: 5, z: 4 })
      // 玻璃 上 (19-25)
      Glass.positionArray.push({ x: -886, y: 56, z: glassZ })
      Glass.scaleArray.push({ x: 0.1, y: 3, z: 4 })
    }
  ...
  // 外圈柱子间的玻璃 (X轴方向: -886 到 -754)
  {
    ...
    segments.forEach(seg => {
      for (let i = 0; i < 8; i++) {
        ...
        // 下
        Glass.positionArray.push({ x: glassX, y: 38, z: 36 })
        Glass.scaleArray.push({ x: 3.4375, y: 3, z: 0.1 })
        // 中
        Glass.positionArray.push({ x: glassX, y: 47, z: 36 })
        Glass.scaleArray.push({ x: 3.4375, y: 5, z: 0.1 })
        // 上
        Glass.positionArray.push({ x: glassX, y: 56, z: 36 })
        Glass.scaleArray.push({ x: 3.4375, y: 3, z: 0.1 })
      }
    })
  ...
  // 连续4扇玻璃
  {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // 下
      Glass.positionArray.push({ x: glassX, y: 38, z: 115 })
      Glass.scaleArray.push({ x: glassScaleX, y: 3, z: 0.1 })
      // 中
      Glass.positionArray.push({ x: glassX, y: 47, z: 115 })
      Glass.scaleArray.push({ x: glassScaleX, y: 5, z: 0.1 })
      // 上
      Glass.positionArray.push({ x: glassX, y: 56, z: 115 })
      Glass.scaleArray.push({ x: glassScaleX, y: 3, z: 0.1 })
    }
  ...
  // 后方窗户玻璃 (Z: -252)
  {
    ...
    segments.forEach(seg => {
      for (let i = 0; i < 8; i++) {
        const glassX = seg.startX + glassOffset + i * step
        // 下
        Glass.positionArray.push({ x: glassX, y: 38, z: -253 })
        Glass.scaleArray.push({ x: 4, y: 3, z: 0.1 })
        // 中
        Glass.positionArray.push({ x: glassX, y: 47, z: -253 })
        Glass.scaleArray.push({ x: 4, y: 5, z: 0.1 })
        // 上
        Glass.positionArray.push({ x: glassX, y: 56, z: -253 })
        Glass.scaleArray.push({ x: 4, y: 3, z: 0.1 })
      }
    })
