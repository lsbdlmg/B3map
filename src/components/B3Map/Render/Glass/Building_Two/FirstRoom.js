...
const create = (Objects, device, world, RAPIER) => {
  {
    ...
    Glass.positionArray = []
    for (let i = 0; i < 4; i++) Glass.positionArray.push({ x: -553.5, y: 12, z: -7 - i * 10 })
    for (let i = 0; i < 6; i++) Glass.positionArray.push({ x: -553.5, y: 12, z: -55.5 - i * 10 })
    Glass.scaleArray = new Array(12).fill({ x: 0.5, y: 7, z: 4.5 })
    Glass.rotationArray = new Array(20).fill({ x: 0, y: 0, z: 0 })
    Glass.textureIndex = new Array(20).fill(0) //占位 不起作用
