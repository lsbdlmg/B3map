...
const create = (Objects, device, world, RAPIER) => {

  {
    ...
    StairCase.positionArray = [{ x: -121.5, y: 51, z: -3.85 }]
  ...
  }
  {
    ...
    //下台阶
    for (let i = 0; i < 8; i++) StairCase.positionArray.push({ x: -113.25, y: 35 + 2 * i, z: 30.7 - 3.4 * i })
    //上台阶
    for (let i = 0; i < 8; i++) StairCase.positionArray.push({ x: -129.75, y: 35 + 16 + 2 * i, z: 6.9 + 3.4 * i })
  ...
  }
  {
    ...
    Wall.positionArray = [{ x: -122.5, y: 48, z: -5.15 }]
...

  {
    ...
    {
      //最底下 4根
      for (let i = 0; i < 4; i++) WindowFrame.positionArray.push({ x: -105.5 - i * 11, y: 39.5, z: -13.4 })
      //上面连着三层 每层4根
      for (let i = 0; i < 4; i++) WindowFrame.positionArray.push({ x: -105.5 - i * 11, y: 56, z: -13.4 })
      for (let i = 0; i < 4; i++) WindowFrame.positionArray.push({ x: -105.5 - i * 11, y: 65, z: -13.4 })
      for (let i = 0; i < 4; i++) WindowFrame.positionArray.push({ x: -105.5 - i * 11, y: 74, z: -13.4 })

      //上面连着三层 横向 4根
      for (let i = 0; i < 4; i++) WindowFrame.positionArray.push({ x: -121.5, y: 51.5 + i * 9, z: -13.4 })
      //楼梯正面
      ...
      for (let i = 0; i < 4; i++) WindowFrame.scaleArray.push({ x: 16.5, y: 0.5, z: 0.5 })
    }

    // 楼梯左侧
    {
      //楼梯左侧
      for (let i = 0; i < 3; i++) WindowFrame.positionArray.push({ x: -138.5, y: 39.5, z: -13.4 + i * 9 })
      for (let i = 0; i < 3; i++) WindowFrame.positionArray.push({ x: -138.5, y: 56, z: -13.4 + i * 9 })
      for (let i = 0; i < 3; i++) WindowFrame.positionArray.push({ x: -138.5, y: 65, z: -13.4 + i * 9 })
      for (let i = 0; i < 3; i++) WindowFrame.positionArray.push({ x: -138.5, y: 74, z: -13.4 + i * 9 })
      //上面连着三层 横向 4根
      for (let i = 0; i < 4; i++) WindowFrame.positionArray.push({ x: -138.5, y: 51.5 + i * 9, z: -4.4 })
      //楼梯左侧
      ...
      for (let i = 0; i < 4; i++) WindowFrame.scaleArray.push({ x: 0.5, y: 0.5, z: 9.5 })
    }
  ...
  }
  {
    ...
    const wall = createGeometry(device, Wall.vertices, Wall.indices)
    Wall.positionArray = [{ x: -102, y: 49, z: 15.6 },]

    Wall.rotationArray = [{ x: 0, y: 0, z: 0 }]
    Wall.scaleArray = [{ x: 3, y: 17, z: 25 }]
