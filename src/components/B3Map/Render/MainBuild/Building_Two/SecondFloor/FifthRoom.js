...
const create = (Objects, device, world, RAPIER) => {
  {
    //门口
    {
      ...
      DoorFrame.positionArray = [
        ...
        { x: -1012.5, y: 44, z: -242 },
        //横
        { x: -1012.5, y: 54.5, z: -236 },
        { x: -1012.5, y: 34.05, z: -236 },
      ]
  ...
  // 1. 门口周围墙体
  {
    ...
    Wall.positionArray = []
    Wall.scaleArray = []
    Wall.positionArray.push({ x: -1012.5, y: 49, z: -318.5 })
    Wall.scaleArray.push({ x: 1.5, y: 17, z: 76 })
  ...
  }
  {
    ...
    Wall.positionArray = []
    Wall.scaleArray = []
    Wall.positionArray.push({ x: -1012.5, y: 49, z: -223 })
    Wall.scaleArray.push({ x: 1.5, y: 17, z: 6.5 })
    Wall.rotationArray = new Array(1).fill({ x: 0, y: 0, z: 0 })
    Wall.textureIndex = [5.1]
    createRigidBodies(Wall.vertices, Wall.indices, Wall.positionArray, Wall.scaleArray, Wall.rotationArray, world, RAPIER)
    Objects.push({ Object: Wall, object: wall })
  }
  {
    ...
    WallTop.positionArray = [{ x: -1012.5, y: 60.5, z: -236 }]
    WallTop.scaleArray = [{ x: 1.5, y: 5.5, z: 6.5 }] // Matches Door size + height adjustment
  ...
  // 窗户区域墙体 (上下墙 - 分内外)
  {
    ...
    windowCenters.forEach(z => {
      // Inner
      Wall.positionArray.push({ x: xBase + 0.25, y: 60, z: z })
      Wall.scaleArray.push({ x: 0.25, y: 6, z: 10.5 })
      Wall.textureIndex.push(5.1)
      Wall.positionArray.push({ x: xBase + 0.25, y: 35, z: z })
      Wall.scaleArray.push({ x: 0.25, y: 3, z: 10.5 })
      Wall.textureIndex.push(5.1)
      // Outer
      Wall.positionArray.push({ x: xBase - 0.25, y: 60, z: z })
      Wall.scaleArray.push({ x: 0.25, y: 6, z: 10.5 })
  ...
  // 间隔墙 (Left/Middle/Right - 分内外)
  {
    ...
    pillars.forEach(p => {
      // Inner
      Wall.positionArray.push({ x: xBase + 0.25, y: 49, z: p.z })
      Wall.scaleArray.push({ x: 0.25, y: 17, z: p.sz })
      Wall.textureIndex.push(5.1)
      // Outer
      Wall.positionArray.push({ x: xBase - 0.25, y: 49, z: p.z })
      Wall.scaleArray.push({ x: 0.25, y: 17, z: p.sz })
  ...
  // 前后墙 (Front & Back Walls)
  {
    ...
    const wall = createGeometry(device, Wall.vertices, Wall.indices)
    Wall.positionArray = [
      { x: -1043, y: 49, z: -394.25 }, // Inner
      { x: -1042, y: 49, z: -394.75 }  // Outer
    ]
    Wall.scaleArray = [
      { x: 29, y: 17, z: 0.25 },
      { x: 31, y: 17, z: 0.25 }
  ...
  // 天花板
  {
    ...
    Ceiling.positionArray = [{ x: -1042, y: 66, z: -305 }]
