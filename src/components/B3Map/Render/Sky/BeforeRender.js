...
const BeforeRender = async (device, format) => {
  ...
  const sky_night_Texture = device.createTexture({
    size: [512, 512, 6],
    format: 'rgba8unorm',
    dimension: '2d',
    usage:
      ...
      GPUTextureUsage.RENDER_ATTACHMENT, // 不写会报错
  });
  const sky_daytime_Texture = device.createTexture({
    size: [1080, 1080, 6],
    format: 'rgba8unorm',
    dimension: '2d',
    usage:
  ...
  //右、左、上、下、前、后
  const skybox_daytime_images = [skybox_daytime_right, skybox_daytime_left, skybox_daytime_top, skybox_daytime_bottom, skybox_daytime_front, skybox_daytime_back]
  const skybox_night_images = [skybox_night_right, skybox_night_left, skybox_night_top, skybox_night_bottom, skybox_night_front, skybox_night_back]

  for (let i = 0; i < 6; i++) {
    device.queue.copyExternalImageToTexture(
      { source: skybox_daytime_images[i] },
      { texture: sky_daytime_Texture, origin: { x: 0, y: 0, z: i } },
      { width: skybox_daytime_images[i].width, height: skybox_daytime_images[i].height }
    );
  }
  ...
  for (let i = 0; i < 6; i++) {
    device.queue.copyExternalImageToTexture(
      ...
      { texture: sky_night_Texture, origin: { x: 0, y: 0, z: i } },
      { width: skybox_night_images[i].width, height: skybox_night_images[i].height }
    );
  }
  ...
  const fsBindGroupLayout = device.createBindGroupLayout({
    ...
    entries: [
      ...
      { binding: 2, visibility: GPUShaderStage.FRAGMENT, texture: { sampleType: 'float', viewDimension: 'cube' } }, // 天空白天
      { binding: 3, visibility: GPUShaderStage.FRAGMENT, texture: { sampleType: 'float', viewDimension: 'cube' } }, // 天空夜晚
    ],
  })
  ...
  const fsGroup = device.createBindGroup({
    ...
    entries: [
      ...
      { binding: 2, resource: sky_daytime_Texture.createView({ dimension: 'cube' }) },
      { binding: 3, resource: sky_night_Texture.createView({ dimension: 'cube' }) },
    ],
  })
  return {
    Sky: Sky,
